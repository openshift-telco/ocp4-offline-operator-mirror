...
parser.add_argument(
    "--operator-channel",
    default="4.5",
    help="Operator Channel. Default 4.5")
...
def extractWhiteListedOperators(redhat_operators_packages):
  try:
    ...
    with open(args.operator_file) as f:
      for operator in f.read().splitlines():
        name = operator.split(",")[0]
        version = operator.split(",")[1]
        operators.append({"name": name, "version": version})

    mod_package_file_data = []
    for operator in operators:
      for c in data:
        if(c["name"].find(operator["name"]) != -1):
          c["desiredVersion"] = operator["version"]
...
def downloadAndProcessManifests(mod_package_file_data_json):
  for c in mod_package_file_data_json:
    ...
    quay_operator_version =  c["default"] if c["desiredVersion"] == "latest" else c["desiredVersion"]
...
def extractRelatedImagesToFile(operatorCsvYaml):
  for channel in operatorCsvYaml:
    if 'relatedImages' in channel['spec']:
      for entry in channel['spec']['relatedImages']:
        if('image' in entry):
